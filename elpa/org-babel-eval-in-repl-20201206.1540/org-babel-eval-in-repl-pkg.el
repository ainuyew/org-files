(define-package "org-babel-eval-in-repl" "20201206.1540" "Eval org-mode babel code blocks in various REPLs."
  '((eval-in-repl "0.9.2")
    (matlab-mode "3.3.6")
    (ess "16.10")
    (emacs "24"))
  :commit "3591f062873de2d64cc6f83b3555d030506e6ee7" :authors
  '(("Takeshi Teshima" . "diadochos.developer@gmail.com"))
  :maintainers
  '(("Takeshi Teshima" . "diadochos.developer@gmail.com"))
  :maintainer
  '("Takeshi Teshima" . "diadochos.developer@gmail.com")
  :keywords
  '("literate programming" "reproducible research" "async execution")
  :url "https://github.com/diadochos/org-babel-eval-in-repl")
;; Local Variables:
;; no-byte-compile: t
;; End:
